...
Streams reference `category_internal_id` → category row. Indexes on `(hash, name)` and trigram GIN on `name` for catalog search (`ILIKE` + `pg_trgm` fuzzy match when the search term is 3+ characters).
