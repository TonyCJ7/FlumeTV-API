...
PostgreSQL data persists in the **`./postgres-data`** directory. Pulling a newer image does not wipe the database unless you delete that folder.
...
| Variable | Default | Description |
| -------- | ------- | ----------- |
| `ID_PREFIX` | `flumetv_` | Prefix for Stremio catalog/meta/stream ids and addon manifest `idPrefixes`. |
...
| **Commits** | Do **not** use `git commit --no-verify`. Husky runs typecheck and lint-staged (ESLint + Prettier on staged TS); fix reported issues instead of skipping hooks. |
...
This repository is licensed under the **[Apache License 2.0](https://www.apache.org/licenses/LICENSE-2.0)**. See [`LICENSE`](LICENSE) for the full text.

> [!NOTE]
> Dependencies in `package-lock.json` remain under their own licenses. This license applies to **this project’s source**, not third-party packages.
