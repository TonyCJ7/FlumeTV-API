...
import { applyProxyToMetaDetail } from "@/core/applyStreamProxy";
...
export async function addonMetaHandler(args: {
  ...
  try {
    ...
    if (streamWithConfig.config_type === CONFIG_TYPE.DIRECT) {
      ...
      const result = await buildDirectMeta(
      ...
      dlog("[STREAM PROXY] wrapMetaResponseWithProxy direct", {
        userId,
        hasProxy,
        videoCount: result.meta.videos?.length ?? 0,
      });

      const wrappedMeta = await applyProxyToMetaDetail(result.meta, { hasProxy });

      return { ...result, meta: wrappedMeta };
    }

    if (streamWithConfig.config_type === CONFIG_TYPE.XTREME) {
      dlog("[META] Building xtreme catalog meta");
      const result = await buildXtremeMeta(
      ...
      dlog("[STREAM PROXY] wrapMetaResponseWithProxy xtreme", {
        userId,
        hasProxy,
        videoCount: result.meta.videos?.length ?? 0,
      });
