...
export function xtreamLiveStreamsFactory(
  ...
  params?: { defaultContainerExtension?: string },
): XtreamFormattedLiveStream[] {
  const defaultExtension = resolvedContainerExtension(params?.defaultContainerExtension);
  ...
  for (const row of rows) {
    ...
    if (!_isEmpty(_trim(ext))) {
      containerExtension = ext;
    } else if (!_isEmpty(defaultExtension)) {
      containerExtension = defaultExtension;
    }
