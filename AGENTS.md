...
- `backend-reference.md` — _implementation_ (what the code does and where).
- `api-documentation.md` — _contracts_ (what clients can rely on at the wire).
- `README.md` — _operations_ (self-hosting only; not API shapes).
...
Pre-commit (husky): `npm run typecheck`, then lint-staged (ESLint + Prettier on staged TS).
